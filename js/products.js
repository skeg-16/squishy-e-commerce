/* ==========================================================================
   ...
   Includes API integration helper functions for dynamic backend fetching.
   ========================================================================== */
...
async function getProductsAsync() {
  if (typeof SquishiesAPI !== 'undefined') {
    const apiProducts = await SquishiesAPI.fetchProducts();
    if (apiProducts && Array.isArray(apiProducts)) {
      const map = {};
      apiProducts.forEach(p => { map[p.slug] = p; });
      return map;
    }
  }
  return PRODUCTS;
...
async function getProductBySlugAsync(slug) {
  if (typeof SquishiesAPI !== 'undefined') {
    const p = await SquishiesAPI.fetchProductBySlug(slug);
    if (p) return p;
  }
